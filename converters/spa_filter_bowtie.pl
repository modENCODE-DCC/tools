  ...
  while (<BOWTIE>) { 
	  ...
	  if ($skip_flag) {
		  ...
		  while ($bowtie_line !~ /\Q$read_id/) {
		      ...
		      #print STDERR "BOWTIE READ: $bowtie_read\n";
		  ...
		  #print STDERR "BOWTIE match at line $lines_read_bowtie: $bowtie[2], $bowtie[1]\n";
		  chomp($bowtie_line);
		  @bowtie = split("\t",$bowtie_line);
		  if (!$skip_flag) {
		      if (!$read1_found) {
			  ...
			  if (scalar(@bowtie_chr_spa) > 1) {
			      $bowtie_chr = $bowtie_chr_spa[1] . "_" . $bowtie_chr_spa[2] . "_" . $bowtie_chr_spa[3] . "_" . $bowtie_chr_spa[6] . "_" . $bowtie_chr_spa[7];
			  } else {
			      $bowtie_chr = $bowtie[2];
			  }
	  ...
	  print STDERR "$lines_read lines processed\n" if ($lines_read % 10000 == 0);
      }
  }
  print STDERR "++++++++++++++++++++++++++++++++++\n";
