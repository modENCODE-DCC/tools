...
($lines_read, $lines_written, $lines_in_file) = &process;
my $stop = time();
my $time = $stop-$start;
...
sub process {
  my %opts = ();
  die("Usage: fasta_merge_sam.pl <fasta> <sam>\n") if (@ARGV == 0 && -t STDIN);
  my $seq_file = $ARGV[0];
  my $sam_file = $ARGV[1];
  ...
  print STDERR "Assuming sequence file \"$seq_file\" is sorted... ";
