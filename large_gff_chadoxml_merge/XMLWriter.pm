...
sub init {
  ...
  foreach my $tempfile (@tempfile_names) {
    $self->get_tempfiles()->{$tempfile} = new IO::File("out_005_${tempfile}.xml", "w+");
  }
}
...
sub combine {
  my ($self) = @_;
  my @tempfile_names = ('dbxrefs', 'cvterms', 'organisms', 'features', 'featurelocs', 'featureprops', 'feature_relationships', 'analyses', 'analysisfeatures', 'attributes', 'protocols', 'wiggle_data', 'data', 'default');
  open OUT, ">out_006_final.xml";
  foreach my $tempfile (@tempfile_names) {
    my $tmpfh = $self->get_tempfiles()->{$tempfile};
    seek($tmpfh, 0, 0) or die "Can't seek to recombine.";
    while (<$tmpfh>) {
      print OUT $_;
    }
    close $tmpfh;
...
sub write_chadoxml {
  ...
  my $id = "experimentx_" . $experiment->get_id();
...
sub write_applied_protocol : RESTRICTED {
  my ($self, $applied_protocol) = @_;
  
  my $id = "applied_protocolx_" . $applied_protocol->get_id();
...
sub write_protocol : RESTRICTED {
  my ($self, $protocol) = @_;
  my $id = "protocolx_" . $protocol->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
...
sub write_experiment_prop : RESTRICTED {
  my ($self, $experiment_prop, $experiment_id) = @_;
  my $id = "experiment_propx_" . $experiment_prop->get_id();
  if ($seen_ids{ident $self}->{$id}++) {
    $self->println("$id");
  } else {
...
sub write_datum : RESTRICTED {
  my ($self, $datum) = @_;
  my $id = "datumx_" . $datum->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
...
sub write_feature {
  my ($self, $feature_cache, $datum_id) = @_;
  my $id = "featurex_" . $feature_cache->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
    ...
    $self->println_to('features', "<data_feature>");
    $self->println_to('features', "<data_id>" . $datum_id . "</data_id>");
    $self->println_to('features', "<feature_id>$id</feature_id>");
    $self->println_to('features', "</data_feature>");
    ...
    foreach my $feature_relationship ($feature->get_relationships(1)) {
      $self->write_feature_relationship($feature_relationship, $datum_id);
    }
    foreach my $feature_location (@{$feature->get_locations()}) {
      $self->write_featureloc($feature_location, $id, $datum_id);
    }
...
sub write_feature_relationship : RESTRICTED {
  my ($self, $feature_relationship, $datum_id) = @_;

  # Make sure the features are written before we start recursing feature_relationship->object->feature_relationship
  $self->write_feature($feature_relationship->get_subject(), $datum_id);
  $self->write_feature($feature_relationship->get_object(), $datum_id);

  my $id = "feature_relationshipx_" . $feature_relationship->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
    ...
    $self->println_to('feature_relationships', "<subject_id>" . $self->write_feature($feature_relationship->get_subject(), $datum_id) . "</subject_id>");
    $self->println_to('feature_relationships', "<object_id>" . $self->write_feature($feature_relationship->get_object(), $datum_id) . "</object_id>");
...
sub write_analysisfeature : RESTRICTED {
...
sub write_featureloc : RESTRICTED {
  my ($self, $featureloc, $feature_id, $datum_id) = @_;
  if ($featureloc->get_srcfeature()) {
    # Make sure this feature is written before we start recursing featureloc->srcfeature->featureloc
    $self->write_feature($featureloc->get_srcfeature(), $datum_id);
  }
  ...
  if ($featureloc->get_srcfeature()) {
    $self->println_to('featurelocs', "<srcfeature_id>" . $self->write_feature($featureloc->get_srcfeature(), $datum_id) . "</srcfeature_id>");
  }
  $self->println_to('featurelocs', "</featureloc>");
}

sub write_featureprop : RESTRICTED {
...
sub write_analysis : RESTRICTED {
  my ($self, $analysis) = @_;
  my $id = "analysisx_" . $analysis->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
...
sub write_organism : RESTRICTED {
  my ($self, $organism) = @_;
  my $id = "organismx_" . $organism->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
...
sub write_wiggle_data : RESTRICTED {
  my ($self, $wiggle_data) = @_;
  my $id = "wiggle_datax_" . $wiggle_data->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
...
sub write_attribute : RESTRICTED {
  my ($self, $attribute) = @_;
  my $id = "attributex_" . $attribute->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
...
sub write_cvterm : RESTRICTED {
  my ($self, $cvterm) = @_;
  my $id = "cvtermx_" . $cvterm->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
...
sub write_cv : RESTRICTED {
  my ($self, $cv) = @_;
  my $id = "cvx_" . $cv->get_id();
  if ($seen_ids{ident $self}->{$id}++) {
...
sub write_dbxref : RESTRICTED {
  my ($self, $dbxref) = @_;
  my $id = "dbxrefx_" . $dbxref->get_id();
  if (!$seen_ids{ident $self}->{$id}++) {
...
sub write_db : RESTRICTED {
  my ($self, $db) = @_;
  my $id = "dbx_" . $db->get_id();
  if ($seen_ids{ident $self}->{$id}++) {
...
sub set_indent : RESTRICTED {
  my ($self, $new_level) = @_;
  $indent{ident $self} = $new_level;
}

sub dec_indent : RESTRICTED {
  my ($self) = @_;
...
sub inc_indent : RESTRICTED {
  my ($self) = @_;
  $indent{ident $self} += $self->get_indent_width();
  return $indent{ident $self};
}

sub get_indent : RESTRICTED {
  my ($self) = @_;
  return $indent{ident $self};
}
