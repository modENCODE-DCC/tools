...
abs.each { |ab|
  ...
  cgb_page.protocols = ([ ab[:prep_page] ] + ab[:qc_pages]).map { |p|
    m = @config["mappings"][p[0]]
    ...
    desc = m.nil? ? (p[1] =~ /:\/\// ? p[1] : "#{@config["cgb_wiki"]["display_base"]}#{p[1].gsub(/ /, '+')}") : m["desc"]
  ...
  cgb_qc_images = ab[:qc_pages].map { |p| p[1] }.sort
